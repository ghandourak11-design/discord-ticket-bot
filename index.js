/**
 * FINAL FULL index.js (with /generate fixed via /linkinvite system)
 *
 * IMPORTANT NOTE:
 * - Discord will ALWAYS show "Invited by <who created the invite>".
 * - A bot CANNOT create an invite "as the user".
 *
 * ✅ Solution implemented:
 *   /generate -> creates an invite (bot creates it) + links it to the user in our system
 *               so OUR tracking credits the user who typed /generate.
 *   /linkinvite <code|link> -> lets a user link ANY existing invite code to themselves (optional)
 *
 * ✅ Slash commands:
 *   /vouches
 *   /invites <user>                 -> ONLY shows invites still in server (our tracking)
 *   /generate                       -> returns a personal invite link credited to you in our tracking
 *   /linkinvite <code|link>          -> link an invite to yourself for credit
 *   /addinvites <user> <amount>     -> Admin only
 *   /resetinvites <user>            -> ONLY allowed role IDs (admins do NOT bypass unless they also have role)
 *   /resetall                       -> Admin only
 ...
 *     where invites = still-in-server only (our tracking)
 *
...
async function registerSlashCommands() {
  ...
  const commands = [
    ...
    new SlashCommandBuilder()
      .setName("generate")
      .setDescription("Generate your personal invite link (credited to you in our tracking)."),
...
client.on("guildMemberAdd", async (member) => {
  try {
    ...
    // ✅ Our credit system: if invite code is linked, credit that owner; else credit invite.inviter
    ...
    const inviterMention = `<@${creditedInviterId}>`;
    await logChannel.send(`${member} has been invited by ${inviterMention} and now has **${still}** invites.`);
  } catch (e) {
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (name === "generate") {
        ...
        return interaction.reply({
          content:
            `✅ Here’s your personal invite link (credited to you in our invites):\n${invite.url}\n\n` +
      ...
      if (name === "linkinvite") {
        ...
        if (!invites) {
          return interaction.reply({
            content: "❌ I need invite permissions to verify invite codes.",
            ephemeral: true,
          });
        }
...
/* ===================== SLASH COMMANDS MUST RESPOND QUICKLY: done via replies/defer above ===================== */

/* ===================== INVITE PANEL PERMISSIONS (cache refresh) ===================== */
