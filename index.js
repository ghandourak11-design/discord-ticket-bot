...
async function denyIfStopped(interactionOrMessage) {
  ...
  if (interactionOrMessage.isChatInputCommand?.() || interactionOrMessage.isButton?.() || interactionOrMessage.isModalSubmit?.()) {
    // Must reply ephemeral if possible
    try {
...
client.once("ready", async () => {
  ...
  for (const guild of client.guilds.cache.values()) {
    ...
    getGuildSettings(guild.id); // ensure settings exist
  }
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    const isOwnerCmd =
      interaction.isChatInputCommand() && (interaction.commandName === "stop" || interaction.commandName === "resume");
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith("ticket_modal:")) {
      ...
      // permissions: opener + staff roles + admins (admins already can see if they have Manage Channels or default perms)
      const overwrites = [
